...
fn example_bin() -> PathBuf {
    ...
    p.join(if cfg!(windows) { "examples\\stream_writer.exe" } else { "examples/stream_writer" })
}
...
fn write_lock_excludes_a_second_process() {
    let dir = temp_kb();
    let bin = example_bin();
    assert!(bin.is_file(), "build the example: cargo build -p sdsearch-core --example stream_writer");
    ...
    assert!(String::from_utf8_lossy(&buf[..nread]).contains("LOCK_ACQUIRED"),
    ...
    assert_eq!(b.status.code(), Some(3),
               "B should receive WouldBlock (exit 3), stdout={}", String::from_utf8_lossy(&b.stdout));
    ...
    let c = Command::new(&bin)
        ...
        .output()
        .unwrap();
    assert_eq!(c.status.code(), Some(0), "C should take the lock after A releases it");
