...
#[derive(serde::Deserialize)]
struct HitId { id: usize }
#[derive(serde::Deserialize)]
struct DocEntry { id: usize, stored: HashMap<String, String> }

fn expected() -> Expected {
    let raw = std::fs::read_to_string(concat!(
        env!("CARGO_MANIFEST_DIR"), "/tests/fixtures/zsl_expected_multiseg.json"
    )).expect("multiseg oracle missing — run tools/gen_zsl_multiseg_fixture.php");
    serde_json::from_str(&raw).unwrap()
}
fn idx() -> ZslIndex {
    ZslIndex::open(&PathBuf::from(concat!(
        env!("CARGO_MANIFEST_DIR"), "/tests/fixtures/zsl_index_multiseg"
    ))).unwrap()
}
fn sorted(mut v: Vec<usize>) -> Vec<usize> { v.sort(); v }
fn eng(hits: &[Hit]) -> Vec<usize> { sorted(hits.iter().map(|h| h.id).collect()) }
fn ora(e: &Expected, k: &str) -> Vec<usize> { sorted(e.queries[k].iter().map(|h| h.id).collect()) }
// RAW order (unsorted) exactly as the engine / oracle returns it — to check ranking coherence.
fn eng_raw(hits: &[Hit]) -> Vec<usize> { hits.iter().map(|h| h.id).collect() }
fn ora_raw(e: &Expected, k: &str) -> Vec<usize> { e.queries[k].iter().map(|h| h.id).collect() }
...
fn term_queries_cross_segment_boundary() {
    ...
    assert_eq!(eng(&term_query(&i, "title", "mysql", 0.0, 100)), ora(&e, "term:title:mysql"));
}
...
fn term_vpn_rank_order_matches_oracle() {
    ...
    assert_eq!(eng_raw(&term_query(&i, "title", "vpn", 0.0, 100)), expected_order);
}
...
fn deleted_doc_term_returns_empty() {
    ...
    assert_eq!(eng(&term_query(&i, "title", "backup", 0.0, 100)), Vec::<usize>::new());
}

#[test]
fn wildcard_fuzzy_phrase_match_oracle() {
    let (i, e) = (idx(), expected());
    assert_eq!(eng(&wildcard_query(&i, "title", "re*", 0, 0.0, 100)), ora(&e, "wildcard:title:re*"));
    assert_eq!(eng(&fuzzy_query(&i, "title", "mysgl", 0.5, 3, 0.0, 100)), ora(&e, "fuzzy:title:mysgl:0.5:3"));
    assert_eq!(eng(&phrase_query(&i, "title", &["how", "to"], 0.0, 100)), ora(&e, "phrase:title:how to"));
}
...
fn stored_routing_by_base_is_correct() {
    ...
    for d in &e.docs {
        ...
        assert_eq!(got.get("id_key"), d.stored.get("id_key"), "id {} mal ruteado", d.id);
    }
}
